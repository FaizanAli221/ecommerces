      ...
      if (orderRes.data.success) {
        ...
        orderRes.data.orders.forEach((order) => {
          (order.items || []).forEach((item) => {
            const product = (productRes.data?.products || []).find(p => p._id === (item.itemId || item._id));

            let resolvedImage = '/default-product.jpg';
            if (item.image) {
              resolvedImage = Array.isArray(item.image) ? item.image[0] : item.image;
            } else if (product?.image) {
              resolvedImage = Array.isArray(product.image) ? product.image[0] : product.image;
            }
            ...
            <div key={index} className="py-4 border-t border-b text-gray-700 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="flex items-start gap-6 text-sm">
                <img className='w-16 sm:w-20 object-cover rounded'
                 src={typeof item.image === 'string' && item.image.startsWith('http') ? item.image : (item.image || '/default-product.jpg')}
