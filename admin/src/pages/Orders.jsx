          ...
          orders.length === 0 ? (
            ...
            orders.map((order, index) => (
                ...
                <div>
                  {(order.items || []).map((item, idx) => (
                    <p className='py-0.5' key={idx}>
                      {item.name || 'Product'} x {item.quantity} <span>({item.size})</span>
                    </p>
                  ))}
                </div>
                <p className='mt-3 mb-2 font-medium'>{order.address ? `${order.address.firstName || ''} ${order.address.lastName || ''}`.trim() : 'Customer'}</p>
                {order.address && (
                  <div>
              ...
              <div>
                ...
                <p>Date : {new Date(order.date || Date.now()).toLocaleDateString()}</p>
              </div>
                <p className='text-sm sm:text-[15px]'>{currency}{order.amount}</p>
                <select onChange={(event) => statusHandler(event, order._id)} value={order.status} className='p-2 font-semibold border'>
